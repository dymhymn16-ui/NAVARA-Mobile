...
{
    m_input->setFixedSize(
        ImVec2(width(), UISettings::keyboardRowHeight()));

    m_input->setPosition(
        ImVec2(0.0f, 0.0f));

    m_layoutLowerENG->setFixedSize(
        ImVec2(
            width(),
            height() - m_input->height() + 50.0f));

    m_layoutLowerENG->setPosition(
        ImVec2(
            0.0f,
            UISettings::keyboardRowHeight()));

    m_layoutUpperENG->setFixedSize(
        ImVec2(
            width(),
            height() - m_input->height() + 50.0f));

    m_layoutUpperENG->setPosition(
        ImVec2(
            0.0f,
            UISettings::keyboardRowHeight()));

    m_layoutNUM->setFixedSize(
        ImVec2(
            width(),
            height() - m_input->height() + 50.0f));

    m_layoutNUM->setPosition(
        ImVec2(
            0.0f,
            UISettings::keyboardRowHeight()));
...
void Keyboard::draw(ImGuiRenderer* renderer)
{
    renderer->drawRect(
        ...
        UISettings::keyboardBackgroundColor(),
        true);
...
void Keyboard::toggleLang()
{
    if (m_activeLayoutType == KeyboardLayout::Type::NUM)
    {
        if (m_isEngMode)
            setActiveLayout(KeyboardLayout::Type::ENG_LOWER);
        else
    ...
    {
        setActiveLayout(KeyboardLayout::Type::NUM);
    }
}
...
KeyboardInput::KeyboardInput()
{
    ...
    m_commandScroll = 0;
    m_commandTouchStartX = 0.0f;
    m_commandTouchCurrentX = 0.0f;
    m_commandDragging = false;
}

static const char* g_CommandList[] =
{
    "/settime",
    "/tune",
    "/id",
...
void KeyboardInput::updateCommandSuggestions()
{
    for (Button* button : m_commandButtons)
    {
    ...
    const size_t commandCount =
        sizeof(g_CommandList) / sizeof(g_CommandList[0]);

    for (size_t i = 0; i < commandCount; i++)
    {
        std::string command = g_CommandList[i];

        if (command.find(m_input) == 0)
        {
            Button* button =
                new Button(command, UISettings::fontSize());

            button->setCallback([this, command]()
            {
...
void KeyboardInput::draw(ImGuiRenderer* renderer)
{
    ...
    renderer->drawLine(
        ...
        ImVec2(
            ...
            divLinePosY),
        ImColor(0xF5, 0x91, 0x32),
        UISettings::outlineSize());

    if (!m_commandButtons.empty())
    {
        float x = absolutePosition().x;
        float y = absolutePosition().y - 50.0f;

        float buttonWidth = 110.0f;
        float buttonHeight = 42.0f;
        float spacing = 5.0f;

        float visibleWidth = size().x;

        for (size_t i = 0; i < m_commandButtons.size(); i++)
        {
            Button* button = m_commandButtons[i];

            float buttonX =
                x +
                ((int)i - m_commandScroll) *
                (buttonWidth + spacing);
            ...
            button->setVisible(
                buttonX + buttonWidth > x &&
                buttonX < x + visibleWidth);
        }
    }
...
void KeyboardInput::touchPopEvent()
{
    if (m_commandButtons.empty())
        return;
...
void KeyboardInput::touchMoveEvent(const ImVec2& delta)
{
    if (m_commandButtons.empty())
        return;

    if (std::fabs(delta.x) < 1.0f)
        return;
    ...
    int maxScroll =
        std::max(
            0,
            (int)m_commandButtons.size() - visibleCount);

    m_commandScroll -=
        (int)(delta.x / 20.0f);

    if (m_commandScroll < 0)
        m_commandScroll = 0;

    if (m_commandScroll > maxScroll)
        m_commandScroll = maxScroll;

    m_commandDragging = true;
}
...
KeyboardLayout::Row::Key* KeyboardLayout::Row::addKey(
    ...
    : Button("")
{
    switch (extended_key)
    {
        case ExtendedKey::LANG:
            this->setCaption(std::string("lang"));
            break;
    ...
    switch (m_extendedKey)
    {
        case ExtendedKey::SHIFT_ON:
            renderer->drawTriangle(
                ImVec2(
                    absolutePosition().x + width() * 0.37f,
                    absolutePosition().y + height() * 0.50f),
                ImVec2(
                    absolutePosition().x + width() * 0.50f,
                    absolutePosition().y + height() * 0.25f),
                ImVec2(
                    absolutePosition().x + width() * 0.63f,
                    absolutePosition().y + height() * 0.50f),
                ImColor(1.0f, 1.0f, 1.0f),
                true);
            ...
            renderer->drawRect(
                ImVec2(
                    absolutePosition().x + width() * 0.44f,
                    absolutePosition().y + height() * 0.50f - 1),
                ImVec2(
                    absolutePosition().x + width() * 0.56f,
                    absolutePosition().y + height() * 0.68f),
                ImColor(1.0f, 1.0f, 1.0f),
                true);
            break;
        ...
        case ExtendedKey::BACKSPACE:
        {
            ...
            points[0] = ImVec2(
                absolutePosition().x + width() * 0.35f,
                absolutePosition().y + height() * 0.50f);

            points[1] = ImVec2(
                absolutePosition().x + width() * 0.45f,
                absolutePosition().y + height() * 0.25f);

            points[2] = ImVec2(
                absolutePosition().x + width() * 0.65f,
                absolutePosition().y + height() * 0.25f);

            points[3] = ImVec2(
                absolutePosition().x + width() * 0.65f,
                absolutePosition().y + height() * 0.65f);
        ...
        case ExtendedKey::SPACE:
            renderer->drawRect(
                ImVec2(
                    absolutePosition().x + width() * 0.07f,
                    absolutePosition().y + height() * 0.35f),
                ImVec2(
                    absolutePosition().x + width() * 0.93f,
                    absolutePosition().y + height() * 0.65f),
                ImColor(1.0f, 1.0f, 1.0f),
                true);
            break;
        ...
        case ExtendedKey::SEND:
            renderer->drawTriangle(
                ImVec2(
                    absolutePosition().x + width() * 0.3f,
                    absolutePosition().y + height() * 0.25f),
                ImVec2(
                    absolutePosition().x + width() * 0.3f,
                    absolutePosition().y + height() * 0.75f),
                ImVec2(
                    absolutePosition().x + width() * 0.7f,
                    absolutePosition().y + height() * 0.50f),
                ImColor(1.0f, 1.0f, 1.0f),
                true);
            break;
...
void KeyboardLayout::Row::Key::touchPopEvent()
{
    ...
    switch (m_extendedKey)
    {
        ...
        case ExtendedKey::SHIFT_ON:
            keyboard->changeCase(
                KeyboardLayout::KeyCase::UPPER);
            break;

        case ExtendedKey::SHIFT_OFF:
            keyboard->changeCase(
                KeyboardLayout::KeyCase::LOWER);
            break;
...
void KeyboardLayoutLowerENG::performLayout()
{
    ...
    m_rows[2]->addKey(ExtendedKey::SHIFT_ON)
        ->setFixedSize(
            ImVec2(
                keySize.x * 1.5f,
                0.0f));
    ...
    m_rows[3]->addKey(std::string("."), '.');
    m_rows[3]->addKey(ExtendedKey::LANG);

    m_rows[3]->addKey(ExtendedKey::SPACE)
        ->setFixedSize(
            ImVec2(
                keySize.x * 3,
                0.0f));
...
void KeyboardLayoutUpperENG::performLayout()
{
    ...
    m_rows[2]->addKey(ExtendedKey::SHIFT_OFF)
        ->setFixedSize(
            ImVec2(
                keySize.x * 1.5f,
                0.0f));
...
KeyboardHistory::KeyboardHistory()
{
    ...
    m_vMessages.resize(
        KEYBOARD_MAX_HISTORY_MESSAGES);
}

void KeyboardHistory::add(
    const std::string& input)
{
    for (
        int i = KEYBOARD_MAX_HISTORY_MESSAGES - 2;
        i > 0;
        i--)
    {
    ...
    if (m_count < KEYBOARD_MAX_HISTORY_MESSAGES)
        m_count++;
}
